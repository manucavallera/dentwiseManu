...
import { es } from "date-fns/locale"; // 🔥 IMPORTANTE: Agregamos locale español
...
export async function getAppointments() {
  try {
    ...
    throw new Error("Error al obtener los turnos"); // TRADUCIDO
  }
}
...
export async function getUserAppointments() {
  try {
    ...
    if (!userId) throw new Error("Debés iniciar sesión para ver tus turnos"); // TRADUCIDO

    const user = await prisma.user.findUnique({ where: { clerkId: userId } });
    if (!user) throw new Error("Usuario no encontrado"); // TRADUCIDO
    ...
    throw new Error("Error al cargar tus turnos"); // TRADUCIDO
  }
}
...
export async function bookAppointment(input: BookAppointmentInput) {
  try {
    const { userId } = await auth();
    if (!userId) throw new Error("No autorizado"); // TRADUCIDO

    if (!input.doctorId || !input.date || !input.time) {
      throw new Error("Faltan datos requeridos"); // TRADUCIDO
    }
    ...
    if (bookedSlots.includes(input.time)) {
      throw new Error(
        "Este horario acaba de ser reservado. Por favor elegí otro.",
      ); // TRADUCIDO
    }
    ...
    const appointment = await prisma.appointment.create({
      data: {
        ...
        time: input.time,
        reason: input.reason || "Consulta General", // TRADUCIDO (Antes "General consultation")
        status: "CONFIRMED",
      },
      include: {
        user: { select: { firstName: true, lastName: true, email: true } },
    ...
    try {
      ...
      await resend.emails.send({
        ...
        to: [user.email],
        subject: "Confirmación de Turno - DentWise", // 🔥 ASUNTO TRADUCIDO
        react: AppointmentConfirmationEmail({
          doctorName: appointment.doctor.name,
          // 🔥 FECHA EN ESPAÑOL
          appointmentDate: format(appointment.date, "EEEE, d 'de' MMMM, yyyy", {
            locale: es,
          }),
          appointmentTime: appointment.time,
          appointmentType: appointment.reason || "Consulta General", // TRADUCIDO
    ...
    throw new Error(error.message || "Error al reservar el turno"); // TRADUCIDO
  }
}
...
export async function updateAppointmentStatus(input: {
  ...
  try {
    ...
    throw new Error("Error al actualizar el turno"); // TRADUCIDO
  }
}
